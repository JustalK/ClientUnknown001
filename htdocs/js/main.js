...
$(function() {
	...
	function parallax() {
		if(positionTopScroll>anchorPosition[0]) {
			var cal = -(positionTopScroll-anchorPosition[0])/5.5;
			$("#parallax").css("background-position","0% "+cal+"px");
		}
		if(positionTopScroll>anchorPosition[2]) {
			var cal = -(positionTopScroll-anchorPosition[2])/5.5;
			$("#temoignages-bg").css("background-position","0% "+cal+"px");
		}
	}
		...
		if(positionTopScroll<prevPosition) {
			for(var i=0;i<anchorName.length;i++) {
				if(anchorPosition[i]==prevPosition) {
					...
					if(i>=1) {
						prevPosition = anchorPosition[i-1];
					} else {
						prevPosition = 0;
					}
					nextPosition = anchorPosition[i];
					break;
				}
			}			
		}
